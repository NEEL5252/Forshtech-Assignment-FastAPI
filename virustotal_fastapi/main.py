...
async def get_virustotal_report(
    ...
    data, fetched = await fetch_virustotal_report_without_redis(endpoint_type, endpoint_value, db)
...
async def refresh_data(request: Request, endpoint_type: str, endpoint_value: str, db: Session = Depends(get_db)):
    data, _ = await fetch_virustotal_report_without_redis(endpoint_type, endpoint_value, db)
